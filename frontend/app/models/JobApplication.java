...
public class JobApplication {
     ...
     */
    public static JobApplication deserialize(JsonNode node) throws Exception {
        try {
            if (node == null) {
            ...
            JobApplication challenge = Json.fromJson(node, JobApplication.class);

            // Extract and set jobPublisher from appliedJob
            if (node.has("appliedJob") && node.get("appliedJob").has("jobPublisher")) {
                JsonNode jobPublisherNode = node.get("appliedJob").get("jobPublisher");


                User jobPublisher = Json.fromJson(jobPublisherNode, User.class); // Assuming User has a matching structure
                challenge.setJobPublisher(jobPublisher);
            }
    ...
    public User getJobPublisher() {return jobPublisher;}

    public void setJobPublisher(User jobPublisher) {this.jobPublisher = jobPublisher;}
