 ...
 */
public class RAJobApplicationService {
    @Inject
    Config config;
    ...
    @Inject
    public RAJobApplicationService(UserService userService) {
     ...
     */
    public RAJob getCurrentJobZone() {
        RAJob currentRAJobZone = null;
        if (session("rajobId") != null && Long.parseLong(session("rajobId")) > 0) {
            currentRAJobZone = getRAJobById(Long.parseLong(session("jobId")));
        }
        return currentRAJobZone;
    }
    ...
    public RAJob getRAJobById(Long rajobId) {
        ...
        try {
            ...
            if (response.has("error")) {
                Logger.debug("RAJobService.getRAJobById() did not get rajob from backend with error.");
                return null;
            }
            ...
            if (rajob.getRajobPublisher() == null) {
                Logger.debug("RAJobService.getRAJobById() creator is null");
                throw new Exception("RAJobService.getRAJobById() creator is null");
            }
        } catch (Exception e) {
    ...
    /**
     * This method intends to get RAJob application by id by calling backend APIs.
     *
     * @param rajobApplicationId
     * @return Job
     */
    public RAJobApplication getRAJobApplicationById(Long rajobApplicationId) {
        RAJobApplication rajobApplication = null;
        try {

            JsonNode response = RESTfulCalls.getAPI(RESTfulCalls.getBackendAPIUrl(config,
                    Constants.GET_RAJOB_APPLICATION_BY_ID + rajobApplicationId));
            if (response.has("error")) {
            ...
            rajobApplication = rajobApplication.deserialize(response);

            System.out.println("jobApplication: "+ rajobApplication);
            if (rajobApplication.getApplicant() == null) {
                Logger.debug("RAJobService.getApplicant() creator is null");
                throw new Exception("RAJobService.getJobById() applicant is null");
            }

            if (rajobApplication.getAppliedRAJob() == null) {
...
//    }
     ...
     *
     * @param RAJobApplicationForm: job registration form
     * @return
     * @throws Exception
     */
    public ObjectNode serializeFormToJson(Form<RAJobApplication> RAJobApplicationForm, Long rajobId) throws Exception {
        ObjectNode jsonData = null;
        System.out.println("rajob application form: " + RAJobApplicationForm.toString());
        try {
            Map<String, String> tmpMap = RAJobApplicationForm.data();
            jsonData = (ObjectNode) (Json.toJson(tmpMap));

            if (RAJobApplicationForm.field("markAsPrivate").value() != null && RAJobApplicationForm.field(
            ...
            RAJob rajob = new RAJob(rajobId);
            jsonData.put("appliedRAJob", Json.toJson(rajob));

        } catch (Exception e) {
            Logger.debug("RAJobApplicationService.serializeFormToJson exception: " + e.toString());
            throw e;
        }
