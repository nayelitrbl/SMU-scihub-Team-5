...
(function($, window) {
  ...
  Starrs = (function() {
    ...
    function Starrs($el, options) {
      ...
      this.createStars();
      this.syncRating();
      this.$el.on('mouseover.starrs', 'span', function(e) {
        return _this.syncRating(_this.$el.find('span').index(e.currentTarget) + 1);
      });
      this.$el.on('mouseout.starrs', function() {
        return _this.syncRating();
      });
      this.$el.on('click.starrs', 'span', function(e) {
...
//      }
    	if (!this.options.rating) {
    	      ...
    	      var starrs = $(".starrs")[0];
    	      var url = "/serviceExecutionLog/updateRating/serviceExecutionLogId/"+$(starrs).attr("data-selid")+"/rate/"+rating;
